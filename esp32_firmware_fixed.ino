...
// Helper function to sanitize and format datetime
String formatDateTime(const String& dateTimeStr) {
  ...
  for (int i = 0; i < cleaned.length(); i++) {
    ...
    // Only keep printable ASCII characters (32-126) and common separators
...
void handleSubmit(const String &payload) {
  ...
  // Sanitize and format the datetime before storing
  g_datetime = formatDateTime(p[11]);
  ...
  if (sendViolation(doc)) {
    ...
    // Use the sanitized datetime for display
